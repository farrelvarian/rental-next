...
export const getStaticPaths = async (context) => {
  const cl = context.params;
  // const { data } = await axios.get(
  //   `${process.env.NEXT_PUBLIC_BASE_URL}${cl}`
  // );
if(cl==="location"){  const dataUser = data.map((item) => ({
  params: { vehicles: item.location.toString() },
}));
// ket: data paths harus sperti dibawah
const paths = [dataUser];
return {
  paths: paths,
  fallback: true,
};}
...
export const getStaticProps = async (context) => {
  ...
  // const { data } = await axios.get(
  //   `${process.env.NEXT_PUBLIC_BASE_URL}${cl}/${vehicles}`
  // );
  return {
    props: {
      user: data,
